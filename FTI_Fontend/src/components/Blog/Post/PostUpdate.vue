<template>
  ...
  <div class="editor__wrapper" v-else>
    ...
    <b-form-checkbox
      v-if="this.$session.getUser().user_type_id==1"
      id="is_public"
      v-model="is_public"
      name="is_public"
      :value="1"
    ...
    <b-button @click="updatePost" block style="margin-top: 1em">Ndrysho Postimin</b-button>
  </div>
</template>
  ...
  methods: {
      async updatePost() {
          try {
            ...
            await this.$api.blog.updatePost(this.$route.params.id, payload)
