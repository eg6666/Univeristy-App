      ...
      <div class="banner">
        ...
        <div>
          <b-button
            ...
            v-if="user_type == 1"
            >Shto Njoftim</b-button
          >
          <b-button
            style="margin-right: 15px"
            @click="notificationsList"
            v-if="user_type == 1"
            >Njoftime FTI</b-button
          >
          <b-button
            style="margin-right: 15px"
            @click="userPostList"
            v-if="user_type == 1"
            >Monitoro postimet</b-button
          >
          <b-button style="margin-right: 15px" @click="userPostList" v-else
            >Postimet e mia</b-button
          >
          <b-button style="margin-right: 15px" @click="createPost"
            >Krijo një postim</b-button
          >
          <b-button
            style="margin-right: 15px"
            v-if="user_type == 1"
            @click="newRegistrations"
            >Regjistrime të reja</b-button
          >
...
export default {
      ...
      this.posts = res.data
        .map((p, i) => {
          return {
            ...
            url:
              p.file_path ||
              `https://source.unsplash.com/300x300?sig=${i}&abstarct`,
          };
        })
        .filter((_, i) => i < 4);
  ...
  methods: {
    ...
    createNotification() {
      this.$router.push({ name: 'FTICreate' });
    },
    notificationsList() {
